...
if __name__ == '__main__':
    ...
    for i in range(len(fullfs_im)):
        ...
        img_pil.save(f'./original/img{i}.png')

        # 保存Groundtruth图像
        gt_np = showgt.detach().cpu().numpy()
        gt_np = (gt_np * 255).astype(np.uint8)
        gt_pil = Image.fromarray(gt_np)
        gt_pil.save(f'./ground/img{i}.png')

        # 保存Binary Mask图像
        mask_np = showmask.detach().cpu().numpy()
        mask_np = (mask_np * 255).astype(np.uint8)
        mask_pil = Image.fromarray(mask_np)
        mask_pil.save(f'./binary/img{i}.png')
